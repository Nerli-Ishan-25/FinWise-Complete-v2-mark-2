...
def run_maintenance():
    ...
    for lang, files in TARGET_FILES.items():
        for file_path in files:
            all_targets.append((lang, file_path))
    ...
    for step in range(1, target_contributions + 1):
        ...
        has_markers = file_has_markers(file_path, lang)
        if has_markers:
            # Action: CLEANUP (Deletion contribution)
            if remove_all_inserted_blocks(file_path, lang):
                ...
                if stage_and_commit(repo, [file_path], commit_msg):
                    ...
                    logger.info(f"Contribution {step}/{target_contributions} (Deletion): Committed cleanup for {os.path.basename(file_path)}")
        else:
            # Action: INSERTION (Insertion contribution)
    ...
    try:
        ...
        if total_committed > 0 or pushed:
            repo.git.config("--local", "maintenance.lastrun", today_str)
            repo.git.config("--local", "maintenance.laststate", final_state)
            repo.git.config("--local", "maintenance.lastcontributions", str(total_committed))
            logger.info(f"Updated Git config: maintenance.lastrun = '{today_str}', maintenance.laststate = '{final_state}', maintenance.lastcontributions = '{total_committed}'")
